...
func dayDistanceSequential(r io.Reader) (float64, error) {
	rdr := csv.NewReader(r)
	total, lNum := 0.0, 0
	for {
...
func dateWorker(date time.Time, ch chan<- result) {
	...
	res.dist, res.err = dayDistanceSequential(resp.Body)
}
...
func main() {
	...
	start := time.Now()
	dist, err := monthDistanceSequential(month)
	if err != nil {
		log.Fatal(err)
	}
